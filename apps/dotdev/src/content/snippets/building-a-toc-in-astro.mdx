...
I noticed that if I moved away from content collections, I could tamper with the rendered HTML a bit more easily [given the extra properties available](https://docs.astro.build/en/guides/markdown-content/#importing-markdown) on direct `.md` imports. I briefly explored rendering a placeholder and then swapping it out in the compiled output, but then ran across something much cleaner.

[This particular section of the MDX docs](https://docs.astro.build/en/guides/integrations-guide/mdx/#using-frontmatter-variables-in-mdx) made me pause and think, "oooh, that _sure looks like_ a property on direct markdown imports...". I rushed to try calling `getHeadings` as a global within the `.mdx`. _It wrote a list of headings. I was in business._
